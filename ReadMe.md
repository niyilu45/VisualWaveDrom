...
- **文字**：在波形区域单击添加文字，也可单击已有文字重新编辑。回车换行，点击“完成”或按 `Ctrl+Enter` 确认。
- **聚焦**：点击后在小卡片中选择多行、多列或矩形，再拖动选择区域。选区显示边框，区外淡化；切换到其他讲解工具时聚焦保持，再点“聚焦”或按 `Esc` 清除。
- **指示**：红圈实时跟随波形区域内的鼠标，单击时出现圆圈震荡提示。
...
- **撤销/重做**：按钮或 `Ctrl+Z` / `Ctrl+Shift+Z`（也支持 `Ctrl+Y`）恢复当前步骤内的标注操作，包括擦除、聚焦、游标和清除。文本框输入使用自身的撤销记录。
