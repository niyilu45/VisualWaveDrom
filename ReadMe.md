...
2. 单击左侧信号行，选择边沿跳转和指定值跳转所使用的信号。每个信号名下方会显示该信号
   在当前工作游标时刻的值。
...
5. 开启 **游标测量** 后，单击波形区域可移动当前工作游标。底部持续显示 A、B、`Δt`
   和 `1/Δt`。
...
2. 选择简化方法并填写目标点数。
3. 点击 **生成简化实例**，上、下两条波形会同时显示原始结果和简化结果。
