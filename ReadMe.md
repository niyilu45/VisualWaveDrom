...
- **指示**：红圈实时跟随波形区域内的鼠标，单击时出现圆圈震荡提示。按住鼠标左键拖动可画临时线，使用当前画笔颜色，也可点击“颜色”更换下一条临时线的颜色；红圈仍保持红色。松开后保留临时线；下一次在画面上单击或按住时，先清除上一条临时线。退出指示、切换步骤或按 `Esc` 也会清除。临时线不影响正式画笔批注，不写入演讲步骤、不进入撤销记录，也不包含在复制图片中。
