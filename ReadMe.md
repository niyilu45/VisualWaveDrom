...
1. 先从电脑中选择 `.txt`、`.csv`、`.tsv`、`.dat` 或 `.log` 数据文件。
...
4. 点击 **生成预览**。文件由服务放入系统临时目录进行解析，页面显示实际 WaveDrom
   效果图、总列数和数据点数；解析完成后临时文件立即删除。
...
- 每个信号可以独立设置波形颜色和整行背景色；还可以用列号、列区间或 A/B 游标范围，
  为指定列设置不同的背景色。
- 模拟显示的信号行底部带有行高调整线；按住并上下拖动即可放大或缩小该行。数字和总线
  模式保持标准行高，重新切回模拟模式时会恢复该信号上次调整的高度。
- 默认只显示简化结果并占满整行；打开 **原始数据** 开关后，上半行显示原始数据，
  下半行显示当前简化结果，可直接进行前后对比。
- 支持缩放、拖动、适合窗口、纵向虚拟滚动和底部总览图。
...
1. 单击左侧信号名称，选择需要设置样式的信号。
2. 在工具栏的 **行样式** 中选择波形颜色或整行背景色；对应的清除按钮可以恢复自动波形色
   或透明行背景。
3. 在 **指定列** 中输入 `1,3-5` 这样的列号，选择颜色后点击 **应用列背景**。点击
   **使用 A-B** 可以自动填入两个游标之间的列；**清除列背景** 只清除输入范围内的覆盖色。

样式会自动保存到信号的 `scope` 字段，并跟随展示实例和覆盖结果保存。列背景采用
`backgroundRanges` 区间存储，`start` 为从 0 开始的包含位置，`end` 为不包含的结束位置，
因此大波形不会为每一列生成一条配置。

### 游标定位和跳转

1. 点击工具栏的 **A** 或 **B**，或直接点击画布中的游标竖线，选择当前工作游标。
...
5. 在“条件”输入框填写 `==1`、`>=10`、`>=10 && <20` 或 `==READY`，再使用左右按钮
   ...
   `==`、`!=`、`>`、`>=`、`<`、`<=`、`&&` 和 `||`。Enter 向后查找，
   Shift+Enter 向前查找。
6. 开启 **游标测量** 后，单击波形区域可移动当前工作游标。游标会吸附到所点信号最近的
